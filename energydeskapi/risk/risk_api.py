...
class RiskApi:
    ...
    @staticmethod
    def get_marketestimators_dict(api_connection: ApiConnection, latest=True) -> Dict[str, pd.DataFrame | list] | None:
        """Fetches market estimators from the database"""
        logger.info("Fetching market estimators")
        if latest:
    ...
    @staticmethod
    def get_create_report(api_connection: ApiConnection, payload):
    ...
    @staticmethod
    def get_market_areas_filtered(api_connection: ApiConnection, areas:list[str]):
        """Fetches market areas from the database"""
        logger.info("Fetching market areas")
    ...
    @staticmethod
    def get_pfe_reports(api_connection: ApiConnection, payload):
    ...
    def post_area_volatility(api_connection: ApiConnection, area_pk: int, volatility_data: list):
        ...
        Args:
            ...
            volatility_data: List of volatility values
        
        Returns:
            Tuple of (success, json_res, status_code, error_msg)
        """
        logger.info(f"Posting volatility data for market area {area_pk}")
        payload = {'data': volatility_data}
        success, json_res, status_code, error_msg = api_connection.exec_post_url(
            f'/api/riskmanager/marketareas/{area_pk}/store_volatility/', 
            payload
        )
        return success, json_res, status_code, error_msg