...
* ``price_area`` (e.g. ``"NO1"`` … ``"NO5"``) is the primary grouping and
  filter dimension for origination views.  Low cardinality (5 Norwegian
  areas), stable, and almost always used as a filter — ideal tag candidate.
  Pass ``""`` when unknown; filter queries such as
  ``r.price_area == "NO2"`` will simply exclude those points.
...
def asset_forecast_point(
    *,
    ...
    price_area:
        Elspot price area, e.g. ``"NO1"`` … ``"NO5"``.  Primary filter and
        grouping dimension for origination views.  Pass ``""`` when unknown.
    bidzone:
