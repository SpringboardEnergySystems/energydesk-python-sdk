...
from energydeskapi.contracts.contracts_api import (Contract, ContractFilter,
                                                   ContractsApi, ContractTag)
from energydeskapi.contracts.masteragreement_api import (
    MasterAgreementApi, MasterContractAgreement)
from energydeskapi.customers.customers_api import CustomersApi
from energydeskapi.customers.users_api import UsersApi
from energydeskapi.geolocation.location_api import LocationApi
from energydeskapi.gos.gos_api import GoContract, GosApi
from energydeskapi.sdk.common_utils import init_api
from energydeskapi.sdk.datetime_utils import (convert_datime_to_locstr,
                                              convert_datime_to_utcstr)
...
def get_contract_filters(api_conn):
    parameter = {"user": 1}
    json_contractfilters = ContractsApi.get_contract_filters(
        api_conn, parameter)
...
def register_contract_tag(api_conn):
    ...
    success, returned_data, status_code, error_msg = ContractsApi.upsert_contract_tag(
        api_conn, contract_tag)
    print(returned_data)


def get_master_contract_agreements(api_conn):
    parameter = {"user": 1}
    json_masteragreement = MasterAgreementApi.get_master_agreements_embedded(
        api_conn)
...
def get_master_contract_agreement_by_pk(api_conn):
    pk = 2
    json_masteragreement = MasterAgreementApi.get_master_agreements_by_key(
        api_conn, pk)
    # print(json_masteragreement)
...
def register_contract_filters(api_conn):
    ...
    success, returned_data, status_code, error_msg = ContractsApi.upsert_contract_filters(
        api_conn, contract_filter)
    print(returned_data)


def register_master_contract_agreement(api_conn, regnmb):
    counterres = CustomersApi.get_company_from_registry_number(
        api_conn, regnmb)
    usrprofile = UsersApi.get_user_profile(api_conn)
    usrcomp = CustomersApi.get_company_from_registry_number(
        api_conn, usrprofile['company_nbr'])
    ...
    master_agreement.contract_owner = "http://127.0.0.1:8001/api/customers/companies/" + \
        str(usrcomp['pk']) + "/"
    master_agreement.counterpart = "http://127.0.0.1:8001/api/customers/companies/" + \
        str(counterres['pk']) + "/"
...
def get_sample_contract(api_conn, commodity):
    yester = (datetime.today() + timedelta(days=-1)
              ).replace(hour=0, minute=0, second=0, microsecond=0)
    dtstr1 = convert_datime_to_utcstr(yester)
    # In order to get the date correct
    dtstr2 = convert_datime_to_locstr(yester, "Europe/Oslo")
...
def register_normal_contract(api_conn):
    deliv_start = (datetime.today() + timedelta(days=100)
                   ).replace(hour=0, minute=0, second=0, microsecond=0)
    deliv_end = (datetime.today() + timedelta(days=400)
                 ).replace(hour=0, minute=0, second=0, microsecond=0)
...
def register_sample_contract(api_conn):
    ...
    res = LocationApi.get_local_areas(
        api_conn, LocationTypeEnum.GOs_OFFER_AREA)
    ...
    go_contract.energy_source = GosApi.get_energysource_by_key(
        api_conn, GosEnergySources.HYDRO)
    go_contract.underlying_source = LocationApi.get_local_area_url(
        api_conn, res[3]['pk'])
