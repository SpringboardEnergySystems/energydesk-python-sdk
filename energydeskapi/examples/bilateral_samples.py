...
from energydeskapi.sdk.pandas_utils import get_winter_profile, get_workweek
...
def get_deliveries(api_conn):
    ...
    df2 = df.pivot_table(index='period_from', columns=[
                         'area', 'counterpart'], values='netpos', aggfunc='sum')

    print(df2)

    df3 = df.pivot_table(index='period_from', columns='area',
                         values='netpos', aggfunc='sum')
    print(df3)
...
def generate_sell_prices(api_conn):
    ...
    for index, row in df.iterrows():
        ...
        for result in res['period_prices']:
            ...
            LemsApi.add_order(
                api_conn, result['period_tag'], result['contract_price'], "NOK", mw, "SELL", "NORMAL", expiry)


def fetch_pricing_configurations(api_conn):
    json_pricing_configurations = BilateralApi.get_pricing_configurations(
        api_conn)
...
def fetch_pricing_configuration_pk(api_conn):
    pk = 2
    json_pricing_configurations = BilateralApi.get_pricing_configuration_by_pk(
        api_conn, pk)
    print(json_pricing_configurations)
...
def generate_adjusted_curve(api_conn):
    success, returned_data, status_code, error_msg = BilateralApi.generate_adjusted_curve_from_config(
        api_conn, 1)
